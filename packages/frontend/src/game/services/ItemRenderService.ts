import type { ItemRenderDefinition } from '@rugged/game'

const ITEM_RENDER_URL = '/assets/item-renders.json'
...
class ItemRenderService {
	private renders = new Map<string, ItemRenderDefinition>()
	...
	getRender(id: string | undefined | null): ItemRenderDefinition | null {
		if (!id) return null
		return this.renders.get(id) ?? null
	}

	getRenderModel(id: string | undefined | null, seedKey?: string | number): ItemRenderModel | null {
		const definition = this.getRender(id)
		return resolveItemRender(definition, seedKey)
	}
	...
	private async fetchRenders(): Promise<void> {
		...
		try {
			const response = await fetch(ITEM_RENDER_URL, { cache: 'no-cache' })
			if (!response.ok) {
				this.loaded = true
				return
			}
			...
			const list = Array.isArray(data)
				? data
				: Array.isArray(data?.itemRenders)
					? data.itemRenders
					: []
			this.renders = new Map(
				list
					.filter((entry: ItemRenderDefinition) => Boolean(entry?.id))
					.map((entry: ItemRenderDefinition) => [entry.id, entry])
			)
		} catch (error) {
			void error
		} finally {
			...
			if (DEBUG_LOAD_TIMING) {
				...
				console.info(`[Perf] item-renders loaded count=${this.renders.size} time=${elapsed.toFixed(1)}ms`)
			}
		}
	}
}

export const itemRenderService = new ItemRenderService()

function resolveItemRender(
	definition: ItemRenderDefinition | null,
	seedKey?: string | number
): ItemRenderModel | null {
